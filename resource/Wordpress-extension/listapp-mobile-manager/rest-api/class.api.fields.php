...
class Template extends WP_REST_Posts_Controller
{
	...
	public function get_reviews(WP_REST_Request $request)
	{
		...
		foreach ($comments as &$item) {
			$countRating = get_comment_meta($item->comment_ID, 'listeo-rating', true);
			$results[] = ["id"=>$item->comment_ID, "rating"=>$countRating, "author_name"=>$item->comment_author, "date"=>$item->comment_date, "content"=>$item->comment_content, "author_email"=>$item->comment_author_email];
		}
		return $results;
	}
