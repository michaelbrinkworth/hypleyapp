...
}else if($currentTemplate == 'listingpro'){
    $typeJobListing = 'features';
}

//tags
$tags = 'case27_job_listing_tags';
//categories
$listtingCates = 'job_listing_category';
if($currentTemplate == 'listingpro'){
    $tags = 'list-tags';
    $listtingCates = 'listing-category';
}
...
if (isset($_POST['submit']) && wp_verify_nonce( $nonce, 'inspireuiteam')) {
    ...
    // return ;
    $result = array(
        'homeLayout' => $homepageLayoutSave['homeLayout'],
            ...
            items: {
                ...
                properties: {
                    ...
                    "layout": {
                        ...
                        "options": {
                            "enum_titles": [
                                ...
                                "One Column", "Two Column", "Three Column",  "Flexible Column",
                                "Listing Align Left", "Listing Align Right", "Banner Slider", "Banner Image",
                                 "Card Vertical (Full Vertical)",
                            ]
                        },
                        "default": 1,
                    ...
                    "tags": {
                        "title": "Tags",
                        "type": "string",
                        'enum': ['', <?php $terms = get_terms($tags); foreach ($terms as $item):
                            echo esc_html($item->term_id). ", ";
                        endforeach;?>],
                        'options': {
                            'enum_titles': ['Choose', <?php $terms = get_terms($tags); foreach ($terms as $item):
                                echo "'" . esc_html($item->name) . "', ";
                            endforeach;?>]
                        },
                        'default': '',
                    ...
                    "categoryListingId": {
                        ...
                        'enum': ['', <?php $terms = get_terms($listtingCates); foreach ($terms as $item):
                            echo esc_html($item->term_id) . ", ";
                        endforeach;?>],
                        'options': {
                            'enum_titles': ['Choose', <?php $terms = get_terms($listtingCates); foreach ($terms as $item):
                                echo "'" . esc_html($item->name ). "', ";
                            endforeach;?>]
                        },
                        'default': '',
